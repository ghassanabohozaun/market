<div id="app" class="max-w-md mx-auto min-h-screen relative pb-24 shadow-2xl bg-gray-50/50 dark:bg-[#0b1121] transition-colors duration-300">
        ...
        <div class="grid grid-cols-2 gap-3 mb-2">
            ...
            <div class="col-span-2 bg-gradient-to-br from-emerald-500 to-teal-700 rounded-[1.5rem] p-6 text-white shadow-xl shadow-emerald-500/20 relative overflow-hidden transition-all duration-300 hover:shadow-emerald-500/40 border border-emerald-400/20">
                ...
                <div class="relative z-10 flex flex-col items-start">
                    <div class="flex items-center gap-2 mb-2 opacity-90 bg-black/10 px-3 py-1.5 rounded-full backdrop-blur-sm border border-white/10">
                        ...
                        <p class="text-sm font-bold">{{ __('market.total_market_debts') }}</p>
                    </div>
                    <div class="flex items-baseline gap-2 mt-1">
                        <h2 class="text-4xl font-black tracking-tight drop-shadow-sm" dir="ltr">{{ number_format($totalDebt, 1) }}</h2>
                        <span class="text-lg font-bold opacity-80">{{ __('market.currency') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-darkCard rounded-[1.25rem] p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-3 transition-colors hover:border-primary/30">
                ...
                <div class="flex flex-col items-start">
                    <p class="text-xs text-gray-500 dark:text-gray-400 font-bold whitespace-nowrap">{{ __('market.active_customers') }}</p>
            ...
            <div class="bg-white dark:bg-darkCard rounded-[1.25rem] p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-3 transition-colors hover:border-primary/30">
                ...
                <div class="flex flex-col items-start">
                    <p class="text-xs text-gray-500 dark:text-gray-400 font-bold whitespace-nowrap">{{ __('market.today_collections') }}</p>
                    <div class="flex items-baseline gap-1 mt-0.5">
                        ...
                        <span class="text-[11px] font-bold text-gray-400">{{ __('market.currency') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="px-4 mb-4">
        <div class="relative group mb-3">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('market.search_customer') }}" class="w-full bg-white dark:bg-darkCard border border-gray-200 dark:border-gray-800 rounded-2xl py-3.5 pr-11 pl-4 focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none transition-all text-gray-800 dark:text-gray-100 shadow-sm placeholder-gray-400">
        </div>
        <div class="flex gap-2 overflow-x-auto pb-1 hide-scrollbar">
            <button wire:click="setFilter('all')" class="whitespace-nowrap px-4 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 text-sm font-bold {{ $filter === 'all' ? 'bg-primary text-white' : 'text-gray-600 dark:text-gray-300 bg-white dark:bg-darkCard' }}">{{ __('market.filter_all') }}</button>
            <button wire:click="setFilter('debt')" class="whitespace-nowrap px-4 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 text-sm font-bold {{ $filter === 'debt' ? 'bg-primary text-white' : 'text-gray-600 dark:text-gray-300 bg-white dark:bg-darkCard' }}">{{ __('market.filter_has_debt') }}</button>
            <button wire:click="setFilter('paid')" class="whitespace-nowrap px-4 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 text-sm font-bold {{ $filter === 'paid' ? 'bg-primary text-white' : 'text-gray-600 dark:text-gray-300 bg-white dark:bg-darkCard' }}">{{ __('market.filter_paid') }}</button>
        </div>
    </div>
    ...
    <div class="px-4 pb-4 relative">
        ...
        <h3 class="text-sm font-bold text-gray-500 dark:text-gray-400 mb-3 px-1">{{ __('market.customers_list') }}</h3>
        ...
        <div class="flex flex-col items-center justify-center py-16 text-gray-400">
            ...
            <p class="font-bold text-gray-600 dark:text-gray-300 text-lg mb-1">{{ __('market.no_customers_added') }}</p>
            <p class="text-sm text-gray-400 dark:text-gray-500">{{ __('market.click_to_add_first_customer') }}</p>
        </div>
        @elseif($customers->isEmpty())
        <!-- No Results -->
        <div class="flex flex-col items-center justify-center py-16 text-gray-400">
            <i class="ph-fill ph-magnifying-glass-minus text-5xl text-gray-300 dark:text-gray-600 mb-3"></i>
            <p class="font-bold text-gray-500 dark:text-gray-400">{{ __('market.no_results') }}</p>
        </div>
        @else
        <div class="space-y-3">
            @php
    ...
    <div x-data="{ show: false }" 
        ...
        <div x-show="show" x-transition.translate.y.bottom class="relative bg-white dark:bg-darkCard w-full max-w-md rounded-t-[2rem] sm:rounded-3xl p-6 shadow-2xl border border-white/10 z-10">
            ...
            <form wire:submit.prevent="addCustomer" class="space-y-4">
                <div>
                    <label class="block text-sm font-bold mb-1.5 text-gray-700 dark:text-gray-300">{{ __('market.name') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-bold mb-1.5 text-gray-700 dark:text-gray-300">{{ __('market.phone_optional') }}</label>
        ...
        <div x-show="show" x-transition.translate.y.bottom class="relative bg-gray-50 dark:bg-dark w-full max-w-md h-[92vh] sm:h-[85vh] rounded-t-[2rem] sm:rounded-3xl flex flex-col shadow-2xl overflow-hidden border border-white/10 z-10">
            ...
            <div class="p-5 bg-white dark:bg-darkCard shadow-sm z-10 relative shrink-0">
                <div class="flex justify-between items-end mb-5">
                    <div>
                        <p class="text-sm font-bold text-gray-500 dark:text-gray-400 mb-1">{{ __('market.current_balance') }}</p>
                        <h3 class="text-3xl font-black tracking-tight {{ $activeCustomer->balance > 0 ? 'text-red-500' : ($activeCustomer->balance < 0 ? 'text-emerald-500' : 'text-gray-800 dark:text-white') }}">
                            {{ number_format(abs($activeCustomer->balance), 1) }} <span class="text-base font-normal opacity-80">{{ __('market.currency') }}</span>
                        </h3>
                    </div>
                                    ...
                                    Swal.fire({
                                        html: `
                                            <div class='flex flex-col items-center'>
                                                ...
                                                <h3 class='text-xl font-black text-gray-800 dark:text-gray-100 mb-2'>{{ __('market.are_you_sure') }}</h3>
                                                <p class='text-sm font-medium text-gray-500 dark:text-gray-400'>{{ __('market.confirm_delete_transaction') }}</p>
                                            </div>
                                        `,
    ...
    <div x-data="{ show: false }" 
        ...
        <div x-show="show" x-transition.translate.y.bottom class="relative bg-white dark:bg-darkCard w-full max-w-md rounded-t-[2rem] sm:rounded-3xl p-6 shadow-2xl border border-white/10 z-10">
            ...
            <form wire:submit.prevent="addTransaction" class="space-y-5">
                <div>
                    <label class="block text-sm font-bold mb-1.5 text-gray-700 dark:text-gray-300">{{ __('market.amount_currency') }} <span class="text-red-500">*</span></label>
                    <div class="relative">
